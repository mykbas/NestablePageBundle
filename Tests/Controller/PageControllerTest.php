...
namespace Mykbas\NestablePageBundle\Tests\Controller;
...
class PageControllerTest extends WebTestCase
{
    ...
    public function testListPages()
    {
        ...
        $this->assertContains('why_songbird', $client->getResponse()->getContent());
    ...
    {
        $client = static::createClient();
        $crawler = $client->request('GET', '/mykbas_page/list');

        // click on contact_us link
        $crawler = $client->click($crawler->selectLink('contact_us')->link());

        // I should see "contact_us"
        $this->assertContains('contact_us', $client->getResponse()->getContent());
    ...
    public function testReorderHomePage()
    {
        ...
        $this->assertContains('menu has been reordered successfully', $client->getResponse()->getContent());
    ...
    {
        $client = static::createClient();
        $crawler = $client->request('GET', '/mykbas_page/1/edit');

        $form = $crawler->selectButton('Edit')->form(['page[slug]' => 'home1']);
        $client->submit($form);

        // go back to the list again and i should see the slug updated
        $crawler = $client->request('GET', '/mykbas_page/list');
        $this->assertContains('home1', $client->getResponse()->getContent());
    }
    ...
    {
        $client = static::createClient();
        $crawler = $client->request('GET', '/mykbas_page/new');
        ...
        // go back to the list again and i should see the slug updated
        $crawler = $client->request('GET', '/mykbas_page/list');
        $this->assertContains('test_page', $client->getResponse()->getContent());
        ...
        $this->assertContains('short content', $client->getResponse()->getContent());
        ...
        $crawler = $client->request('GET', '/mykbas_pagemeta');

        $this->assertNotContains('test page title', $client->getResponse()->getContent());
    }
    ...
    public function testDeleteContactUsPage()
    {
        ...
        $crawler = $client->request('GET', '/mykbas_page/5');

        $form = $crawler->selectButton('Delete')->form();
        $crawler = $client->submit($form);
        $crawler = $client->followRedirect();

        $this->assertNotContains('contact_us', $client->getResponse()->getContent());

        // we now connect to do and make sure the related pagemetas are no longer in the pagemeta table.
        $res = $client->getContainer()->get('doctrine')
            ->getRepository('MykbasNestablePageBundle:Page')->findBy(['id' => 5]);
